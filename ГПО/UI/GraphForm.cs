...
namespace MathApp.UI
{
    public class GraphForm : Form
    {
        ...
        // Для статичной отрисовки - сохраняем ВСЕ исходные точки
        private List<PointF> _worldPoints = new List<PointF>(); // Мировые координаты (x,y)
        ...
        public void SetDataDirectly(List<double> timeValues, List<double> values)
        {
            ...
            UpdateWorldPoints(); // Обновляем мировые точки
        ...
        private void InitializeComponent()
        {
            ...
            MouseWheel += (s, e) => { Zoom(e.Delta > 0 ? 0.9 : 1.1, PointToClient(Cursor.Position)); };
        ...
        // Обновляем мировые координаты из исходных данных
        ...
        private void Recalculate()
        {
            ...
            UpdateWorldPoints(); // Обновляем мировые точки после пересчета
            if (_autoScale) AutoScale();
            _needsRedraw = true;
        }
        ...
        private void DrawToBuffer()
        {
            ...
            if (_worldPoints.Count > 1)
                DrawGraphStatic(left, right, top, bottom); // Используем статическую отрисовку
            else
        ...
        // НОВЫЙ МЕТОД: Статическая отрисовка графика
        // Рисует линию строго по исходным точкам, без передискретизации
        private void DrawGraphStatic(int left, int right, int top, int bottom)
        {
            // Преобразуем мировые координаты в экранные
            List<PointF> screenPoints = new List<PointF>();

            foreach (var worldPoint in _worldPoints)
            {
                double x = worldPoint.X;
                double y = worldPoint.Y;

                // Проверяем, попадает ли точка в видимую область (с небольшим запасом)
                if (x < _viewXMin - (_viewXMax - _viewXMin) * 0.1 ||
                    x > _viewXMax + (_viewXMax - _viewXMin) * 0.1)
                    continue;
            ...
            // Рисуем линию, соединяя ВСЕ исходные точки ПО ПОРЯДКУ
            using (var pen = new Pen(_lineColor, 2))
            {
                // Важно: рисуем последовательно все точки, как они идут в исходных данных
            ...
            // Опционально: рисуем маркеры на точках (если их немного)
            if (screenPoints.Count < 200)
            {
                foreach (var p in screenPoints)
                {
                    using (var brush = new SolidBrush(Color.FromArgb(180, _lineColor)))
                        _backGraphics.FillEllipse(brush, p.X - 3, p.Y - 3, 6, 6);
                }
            }
        }
        ...
        private void DrawAxes(int left, int right, int top, int bottom)
        {
            ...
            using (var arrowPen = new Pen(Color.White, 2))
            {
                ...
                if (_viewYMin <= 0 && _viewYMax >= 0)
                {
                    ...
                    DrawArrowHead(_backGraphics, right, yZero, 0, arrowPen);
                    _backGraphics.DrawString("t", new Font("Segoe UI", 11, FontStyle.Italic), Brushes.White, right + 5, yZero - 8);
                }
                else
                {
                    int yAxisPos = bottom;
                    _backGraphics.DrawLine(pen, left, yAxisPos, right, yAxisPos);
                    DrawArrowHead(_backGraphics, right, yAxisPos, 0, arrowPen);
                    _backGraphics.DrawString("t", new Font("Segoe UI", 11, FontStyle.Italic), Brushes.White, right + 5, yAxisPos - 8);
                }
                ...
                if (_viewXMin <= 0 && _viewXMax >= 0)
                {
                    ...
                    DrawArrowHead(_backGraphics, xZero, top, 270, arrowPen);
                    _backGraphics.DrawString("y", new Font("Segoe UI", 11, FontStyle.Italic), Brushes.White, xZero + 5, top - 15);
                }
            }
        }

        private void DrawArrowHead(Graphics g, int x, int y, int angle, Pen pen)
        {
            int arrowSize = 8;
            Point[] arrowPoints;
